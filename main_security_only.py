# main_baseline.py - Security-only 消融训练脚本（FIFO队列 + 强制安全约束）
# 使用修复后的 MEC_Env_FIFO.py

from MEC_Env_FIFO_Security import MEC
...
if __name__ == "__main__":
    ...
    for episode in range(Config.N_EPISODE):
        ...
        while not done:
            ...
            if done:
                ...
                Security_blocked_list.append(getattr(env, 'security_blocked_count', 0))
                ...
                print(f"安全违规数: {security_count} ✅ (仅安全约束)")
                print(f"安全阻断数: {getattr(env, 'security_blocked_count', 0)}")
    ...
    results = {
        ...
        'Security_Blocked': Security_blocked_list,
        'Load_CV': Load_CV_list,
        'Drop_Rate': Drop_rate_list
    }
    
    np.save('security_only_results.npy', results)
    print("\n结果已保存到 security_only_results.npy")
    ...
    axes[1, 0].set_title('Security Violations over Episodes')
    ...
    plt.tight_layout()
    plt.savefig('security_only_training_results.png', dpi=300)

    plt.show()
print("drop_ue_count =", env.drop_ue_count)
print("drop_edge_count =", env.drop_edge_count)
print("drop_trans_count =", env.drop_trans_count)
print("security_blocked_count =", env.security_blocked_count)
