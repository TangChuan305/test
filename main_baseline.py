# main_baseline.py - QECO Baseline训练脚本（使用FIFO队列，不考虑安全）
...
if __name__ == "__main__":
    print("="*70)
    print("QECO Baseline 训练脚本")
    print("="*70)
    print("特性：")
    print("❌ 使用FIFO队列（先进先出，不考虑紧急程度）")
    ...
    for episode in range(Config.N_EPISODE):
        ...
        while not done:
            ...
            if done:
                ...
                print(f"安全违规数: {security_count} ❌ (Baseline不考虑安全)")
    ...
    plt.tight_layout()
    plt.savefig('baseline_training_results.png', dpi=300)
    print("训练图表已保存到 baseline_training_results.png")
