...
class MEC:
    ...
    def get_edge_load_factor(self):
        edge_loads = np.zeros(self.n_edge)

        for edge_index in range(self.n_edge):
            total_bits = 0.0

            for ue_index in range(self.n_ue):
                q = self.edge_computation_queue[ue_index][edge_index]

                # 兼容 queue.Queue 和 list 两种队列实现
                if hasattr(q, "queue"):  # queue.Queue
                    task_list = list(q.queue)
                else:  # 普通 list
                    task_list = list(q)

                # 队列里等待的任务
                for task in task_list:
                    if isinstance(task, dict) and 'SIZE' in task:
                        size = task['SIZE']
                        if size is not None and not np.isnan(size):
                            total_bits += size

                # 当前正在边缘执行的任务
                remain = self.edge_process_task[ue_index][edge_index]['REMAIN']
                if isinstance(remain, (int, float, np.floating)) and not np.isnan(remain) and remain > 0:
                    dens = self.edge_process_task[ue_index][edge_index].get('DENS', 1.0)
                    if dens and dens > 0:
                        total_bits += remain / dens

            edge_loads[edge_index] = total_bits / max(self.comp_cap_edge[edge_index], 1e-6)
