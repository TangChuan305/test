# main_rosco.py - RoSCo训练脚本（使用安全约束和优先级队列）
# 使用修复后的 MEC_Env.py
...
if __name__ == "__main__":
    ...
    for episode in range(Config.N_EPISODE):
        ...
        while not done:
            ...
            if done:
                ...
                print(f"安全违规数: {security_count}")
                print(f"被阻止的不安全动作: {env.security_blocked_count}")
    ...
    np.save('rosco_results.npy', results)
    print("\n结果已保存到 rosco_results.npy")
    ...
    plt.tight_layout()
    plt.savefig('rosco_training_results.png', dpi=300)
    print("训练图表已保存到 rosco_training_results.png")
